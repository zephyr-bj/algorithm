                ...
                loff_t *f_pos)
{
         ...
         * consumer side ignores it, and it will keep other data from interleaving with ours.
         */
...
shortp_out_sem 
    CONTROL the access to the circular buffer
    COPY data from user space to circular buffer
    ONLY MOVE the head pointer, when get the pointer

shortp_out_lock
    CONTROL the access to variables used on the consumer side of the output buffer
    CONTROL the access to hardware address, say "outb, shortp_base+SP_DATA" "inb, shortp_base + SP_STATUS" (so workqueue and timer spin for it)
    CONTROL the access to shortp_output_active (so (*write) spin for it)
    ONLY MOVE the tail pointer, when get the lock
    
shortp_output_active
    INITIATED to be 0
    SET to 1 by the (*write) function when it is 0, and obtain the shortp_out_lock
    CLEARED to 0 by workqueue function when buffer empty: tail == head
    interrupt handler exit immediately when it is 0
    timer handler exit immediately when it is 0
    (*release) will wait it to be 0 before exits
*****/
...
shortp_workqueue
    CALLED by (*write) to kick off
    CALLED by interrupt handler
    
shortp_timer
    ADDED and RESETE by workqueue 
    timeout when missed interrupt
*****/
...
shortp_in_queue
    COPY data to user space, when the wait for it returns
    MOVE ONLY the tail pointer
*****/
